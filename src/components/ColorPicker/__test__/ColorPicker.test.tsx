...
describe('ColorPicker component', () => {
    it.only('renders without crashing', () => {
        ...
        expect(container.find('input')).toBeInTheDocument()
    })
    //测试ui，界面结构是否正确
    ...
    //测试行为
    it('should send the correct event when color changed', async () => {
        render(<ColorPicker />)
    ...
    it('should send the correct event when click the color list', () => {
        const { container } = render(<ColorPicker />)
        const input = screen.getByRole('input')
        const firstItem = container.querySelector(
            'li:first-child div',
